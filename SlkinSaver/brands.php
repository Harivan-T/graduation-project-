<?php $br_id = isset($_GET['brand']) ? (int)$_GET['brand'] : 0;?>
<!-- Product List Start -->
    ...
    <div class="container bg-white pt-4">
        <div class="row">
            <div class="col-lg-8">
                <div class="row">
                    <?php 
                    $nbOfResults = $dbs->query("SELECT COUNT(*) FROM products WHERE br_id=$br_id")->fetchColumn();
                        if($nbOfResults>0){
                            $entitiesPerPage = 9; 
                            ...
                            if ($currentPage !== 1) {
                                $pagination[] = [
                                    'page' => 'Previous',
                                    'link' => '?brand='.$br_id.'&pg=' . ($currentPage - 1),
                                    'active' => false,
                                ];
                            }
                            ...
                            $productQuery = $dbs->query(@"
                                        SELECT products.*,images.* 
                                            ...
                                            WHERE br_id=$br_id
                                                ORDER BY br_id DESC LIMIT $entitiesPerPage OFFSET $to ");
                            ...
                            echo "<div class='alert alert-danger m-2 col-md-12' role='alert'>No products for this brand!.</div>";
                        }
                    ?>  
                </div>
            </div>
